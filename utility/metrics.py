...
This file is used to evaluate the performance of the SGL(e.g. recall, ndcg, precision, hit)
"""
import numpy as np

def HR(rank, k, ground_truth):
    rank = rank[:, :k]
    hit_n = 0
    for i in range(len(rank)):
        hits = [1 if item in ground_truth[i] else 0 for item in rank[i]]
        if np.sum(hits) > 0:
            hit_n += 1
